...
class COFS {
    ...
    init() {
        ...
        this.fixedTxTmr = setInterval(async () => {
            __logger.info('FIXED TXTIME REACHED');
            console.log('FIXED TXTIME REACHED')
            await this.send()
                ...
                for (const io of device.io) {
                    if (io.publish.enabled == true) {
                        ...
                        if (!moduleFound) {
                            ...
                            console.log(`New module found __routerStatus getting bigger ${io.publish.moduleId} size ${__routerStatus.length}`);
                            change = true;
                        }
            ...
            try {
                    for (const o of __routerStatus) {
                        ...
                        if (this.rateLimitTxCount < 0) {
                            ...
                            console.log(`!!!! RATE LIMITING INTRODUCED !!!!`);
                            __logger.warn(`RATE LIMIT IS ${this.rateLimitTxCountSP} TRANSMITS EVERY ${this.rateLimitTmrSP} SECONDS`);
                            console.log(`RATE LIMIT IS ${this.rateLimitTxCountSP} TRANSMITS EVERY ${this.rateLimitTmrSP} SECONDS`);
                        } else {
                            if (o.moduleId > -1) {
