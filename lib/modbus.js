...
function parseResponseTcp(buf) {
    ...
    if (buf.length > 9) {
        try {
            ...
            console.log('parseResponseTcp Error', error.message);
        }
    }
...
function Tcp(ipAddress, port) {
    ...
    rtn.onData = (blah) => { console.log('blah',blah)}

    client.on('data', (res) => {
        console.log('shane', res)
        rtn.onData(res)
    })
